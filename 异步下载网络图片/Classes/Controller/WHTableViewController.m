...
//MARK:判断操作是否存在,如果不存在,就去下载,如果存在,直接返回
...
//          [NSThread sleepForTimeInterval:arc4random_uniform(5)];
    ...
    //MARK:保存到沙盒
        /**
         writeToFile : 写入的路径
         atomically: 是否以原子性的操作去写入
         */
        [data writeToFile:[info.icon appendCachePath] atomically:true];//ture 缓存完成后，一次写入
    ...
    //MARK:图片存到模型里，不方便清除内存
            //先更改数据: 将下载回来的图片与模型进行一一对应,不能直接在这个地方更改cell,因为图片下载完成之后,cell里面显示的模型可能已经不是之前显示的模型的内容了.
            //info.image = image;
    //MARK:将图片缓存到字典中,方便清除内存
                
                if(image != nil)
                {
                    [self.imageCache setObject:image forKey:info.icon];
    ...
    // 返回cell的方法调用很频繁 ,所以没有必要每一次都去初始化一个队列,所以用全局
